...
import com.zhenai.lib.core.slang.impl.*;
...
import com.zhenai.lib.core.slang.api.BinaryExpressionTree.Operator;
...
import com.zhenai.lib.core.slang.api.ImportDeclarationTree;
...
import static com.zhenai.lib.core.slang.api.LoopTree.LoopKind.DOWHILE;
import static com.zhenai.lib.core.slang.api.LoopTree.LoopKind.FOR;
import static com.zhenai.lib.core.slang.api.LoopTree.LoopKind.WHILE;
...
import static com.zhenai.lib.core.slang.api.ModifierTree.Kind.PUBLIC;
...
class KotlinTreeVisitor {
    ...
    private Tree createElement(@Nullable PsiElement element) {
        ...
        return convertElementToAST(element, metaData);
    }
    ...
    private Tree convertElementToAST(PsiElement element, TreeMetaData metaData) {
        ...
        } else if (element instanceof KtObjectDeclaration) {
    ...
    private Tree createObjectDeclarationTree(TreeMetaData metaData, KtObjectDeclaration element) {
        List<Tree> childrens = new ArrayList<>();
        for (PsiElement psiElement : element.getBody().getChildren()) {
            childrens.add(convertElementToAST(psiElement, getTreeMetaData(psiElement)));
        }
        return new ObjectDeclarationImpl(metaData, childrens);
    }
    ...
    private Tree createTopLevelTree(PsiElement element, TreeMetaData metaData) {
        ...
        KotlinNativeKind fileAnnotationKind = new KotlinNativeKind(KtFileAnnotationList.class, metaData.textRange());
    ...
    private Tree createReturnTree(TreeMetaData metaData, KtReturnExpression ktReturnExpression) {
        ...
        if (label != null) {
            return createNativeTree(metaData, new KotlinNativeKind(ktReturnExpression, metaData.textRange(), label.name()), ktReturnExpression);
        }
    ...
    private Tree createClassDeclarationTree(TreeMetaData metaData, KtClass ktClass) {
        ...
        Tree nativeClassDecl = createNativeTree(metaData, new KotlinNativeKind(ktClass, metaData.textRange()), children);
    ...
    private Tree convertElementToNative(PsiElement element, TreeMetaData metaData) {
        if (element instanceof KtDestructuringDeclarationEntry || isSimpleStringLiteralEntry(element)) {
            ...
            return createNativeTree(metaData, new KotlinNativeKind(element, metaData.textRange(), element.getText()), element);
        } else {
            return createNativeTree(metaData, new KotlinNativeKind(element, metaData.textRange()), element);
        }
    }
    ...
    private Tree createFunctionDeclarationTree(TreeMetaData metaData, KtFunction functionElement) {
        if (functionElement.getReceiverTypeReference() != null) {
            ...
            return createNativeTree(metaData, new KotlinNativeKind(functionElement, metaData.textRange()), functionElement);
        }
        ...
        boolean isLiteral = functionElement instanceof KtFunctionLiteral;
        FunctionDeclarationTreeImpl impl = new FunctionDeclarationTreeImpl(metaData, modifiers, isConstructor, returnType, identifierTree, parametersList, (BlockTree) bodyTree, nativeChildren);
        impl.isLiteral = isLiteral;
        return impl;
    }
        ...
        return Arrays.stream(KtTokens.MODIFIER_KEYWORDS_ARRAY)
                ...
                .map(element -> {
                    ...
                    if (KtTokens.PUBLIC_KEYWORD.getValue().equals(element.getText())) {
                        ...
                        NativeKind modifierKind = new KotlinNativeKind(element, metaData.textRange(), element.getText());
    ...
    private Tree createIfTree(TreeMetaData metaData, KtIfExpression element) {
        ...
        if (thenBranch == null) {
            ...
            return createNativeTree(metaData, new KotlinNativeKind(element, metaData.textRange()), children);
        }
    ...
    private Tree createParameter(TreeMetaData metaData, KtParameter ktParameter) {
        ...
        if (nameIdentifier == null) {
            return createNativeTree(metaData, new KotlinNativeKind(ktParameter, metaData.textRange()), ktParameter);
        }
    ...
    private Tree createMatchCase(TreeMetaData metaData, KtWhenEntry element) {
        ...
        if (!element.isElse()) {
            ...
            conditionExpression = createNativeTree(treeMetaData, new KotlinNativeKind(KtWhenCondition.class, treeMetaData.textRange()), conditionsList);
        }
    ...
    private Tree createLoopTree(TreeMetaData metaData, KtLoopExpression ktLoopExpression) {
        ...
        if (ktLoopExpression instanceof KtForExpression) {
            ...
            Tree condition = createNativeTree(conditionMetaData, new KotlinNativeKind(ktLoopExpression,textRange), Arrays.asList(parameter, range));
    ...
    private Tree createBinaryExpression(TreeMetaData metaData, KtBinaryExpression element) {
        ...
        if (leftOperand == null || rightOperand == null) {
            ...
            List<Tree> children = Stream.of(leftOperand, rightOperand)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList());
            return createNativeTree(metaData, new KotlinNativeKind(element, metaData.textRange(), operationToken), children);
        }
    ...
    private Tree createUnaryExpression(TreeMetaData metaData, KtUnaryExpression element) {
        ...
        if (operand == null || operator == null) {
            NativeKind nativeKind = new KotlinNativeKind(element, metaData.textRange(), element.getOperationReference().getReferencedNameElement().getText());
            return operand == null
    ...
    private Tree createNativeOperationExpression(TreeMetaData metaData, KtOperationExpression operationExpression) {
        NativeKind nativeKind = new KotlinNativeKind(operationExpression, metaData.textRange(), operationExpression.getOperationReference().getReferencedNameElement().getText());
